...
Cyberdocs_Classifier<- function(raw_text) {
  # SETTING UP LOGGERS 
  ...
  CatchError(load(paste(apiBase, "models/cyberdocs_model", sep = "/")))
  CatchError(source(paste(apiBase,"utility/CleanText.R", sep = "/")))

  cyberdocs_model <- cyberdocs_model3
  ...
  futile.logger::flog.info(msg = paste0("Initial input has ", initdocs, " document(s) containing", initwords, " words."))
  
  topics <- c("IT_NONCYBERSECURITY", "CYBERSECURITY", "NON_IT")
  ...
  rem<-c(as.character(lexicon::pos_df_pronouns[,1]),
         ...
         as.character(lexicon::sw_python), as.character(lexicon::available_data()),
         "free", "encyclopedia", "," ,"may", "also",  "first", 
         "may", "original")
  
  
  DFM_NEW<- CatchError(quanteda::dfm(as.character(test), tolower = TRUE, stem = TRUE, select = NULL, 
                          remove = NULL,
  ...
  alignDFM_NEW<- stm::alignCorpus(DFM_NEW, cyberdocs_model$vocab, verbose = TRUE)
  ...
  new_class<- CatchError(stm::fitNewDocuments(model = cyberdocs_model, documents =alignDFM_NEW$documents, 
                                   newData = DFM_NEW$meta))
 
  colnames(new_class$theta)<- topics
  
  pred<- data.frame(new_class$theta)
  pred<- pred[order(names(pred))]      # Order names by alphabetical order
  
  cyberdocs_classifier<- pred
