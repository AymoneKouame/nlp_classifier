...
Industry_ClassifierTrain<- function(raw_text)  {
  ...
  rem<- c("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", 
        ...
        "year", "yaw", "yards", "yang", "yale", "xx", "xvii", "xv", "xiv", "xiii", "xi", "xavier", "wyoming", "wwii")
  
  print("Creating DFM (Document Feature Matrix)...")
  industry<- gsub("([A-Za-z]+).*", "\\1", raw_text[,1])
  
  DFM<- quanteda::dfm(as.character(train), tolower = TRUE, stem = FALSE, select = NULL, remove = rem,
          dictionary = NULL, thesaurus = NULL, valuetype = "glob", groups = as.factor(industry),
  ...
  PrepDFM<- stm::prepDocuments(DFM$documents, meta = DFM$meta, vocab = DFM$vocab, lower.thresh = 1,
                        upper.thresh = round(length(as.factor(industry))/2), subsample = NULL, verbose = TRUE)

  print("Modeling with stm algorithm...")
  
  # MODEL

  industry_model<- stm::stm(PrepDFM$documents, PrepDFM$vocab, length(PrepDFM$documents), 
  ...
  save(industry_model, file = paste(apiBase, "models/industry_model", sep = "/"))
